...
| [10-the-newsfeed-target-type-names-the-wrong-field.md](./10-the-newsfeed-target-type-names-the-wrong-field.md) | Newsfeed: NewsTarget declares `platform`, the filter reads `platforms`, so an item built from the type is silently discarded | Open, not scheduled |
